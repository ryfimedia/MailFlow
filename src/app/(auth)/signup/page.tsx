...
export default function SignupPage() {
  const { emailPasswordSignUp, signInWithGoogle } = useAuth();
  ...
  const onSubmit = async (data: SignupFormValues) => {
    ...
    const { error } = await emailPasswordSignUp(data.email, data.password);
    if (error) {
      toast({
        variant: 'destructive',
  ...
  return (
    <Card className="w-full max-w-md">
      <CardHeader>
        ...
        <CardDescription>Get started with Ryfi MailFlow by creating your account.</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
